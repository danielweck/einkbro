...
import info.plateaukao.einkbro_.EinkBro_Application
import info.plateaukao.einkbro_.R
import info.plateaukao.einkbro_.activity.EpubReaderActivity
...
object HelperUnit {
    ...
    fun getStringFromAsset(fileName: String): String =
        EinkBro_Application.instance.assets.open(fileName).bufferedReader().use { it.readText() }
    ...
    suspend fun upgradeToLatestRelease(
        ...
        val url = "https://api.github.com/repos/plateaukao/einkbro_/releases"
    ...
    suspend fun upgradeFromSnapshot(
        ...
        val url =
            "https://nightly.link/plateaukao/einkbro_/workflows/buid-app-workflow.yaml/main/app-arm64-v8a-release.apk.zip"
        val request = Request.Builder().url(url).build()
        try {
    ...
    fun loadAssetFile(fileName: String): String {
        ...
        try {
            val jsContent = EinkBro_Application.instance.assets.open(fileName).bufferedReader().use { it.readText() }
